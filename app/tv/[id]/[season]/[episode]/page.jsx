 ...
 return tvData && (
        <div className="relative">
            ...
            <div className="z-[2] pt-4 md:pt-10 pb-20">
                ...
                <div className="w-[90%] p-4 md:p-0 max-w-screen-xl mx-auto md:mb-4 md:mt-4">
                     ...
                     {Array.from({ length: tvData.seasons[seasonRef == 0 ? seasonRef : seasonRef -1].episode_count }).map((_, index) => (
                        <Link key={index} href={`/tv/${id}/${seasonRef}/${index + 1}`}>
                            <div className="flex relative border-b-2 border-gray-400 pb-4 cursor-pointer gap-x-4">
                                <img src={'https://image.tmdb.org/t/p/original' + tvData.seasons[seasonRef == 0 ? seasonRef : seasonRef -1].poster_path} 
                                className="w-12"/>
                                <span className="absolute bottom-4 left-0 bg-red-500 px-2 py-1 font-bold ">{index + 1}</span>
                                <p className="line-clamp-3 text-white">{tvData.seasons[seasonRef == 0 ? seasonRef : seasonRef -1].overview || `${tvData.name} S${seasonRef}E${index+1}`}</p>
                            </div>
                        </Link>
                     ))}
                </div>

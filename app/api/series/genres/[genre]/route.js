...
export async function GET(request, { params }) {
  ...
  try {
    const response = await fetch(
      `https://api.themoviedb.org/3/discover/tv?include_adult=false&include_video=false&language=en-US&page=1&sort_by=${sortBy}&with_genres=${genresId[genre]}`,
      options
    );
    if (!response.ok) {
    ...
    const first20Results = data.results.slice(0, 20);
    return NextResponse.json(first20Results);
  } catch (e) {
    console.log(e);
